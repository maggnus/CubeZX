...
struct CubeMove: Hashable, Codable, Identifiable {
    ...
    init(face: CubeMoveFace, direction: CubeMoveDirection, timestamp: Date = Date()) {
        self.moveType = .face(face)
        self.direction = direction
        self.timestamp = timestamp
    }
    
    init(slice: CubeSliceMove, direction: CubeMoveDirection, timestamp: Date = Date()) {
        self.moveType = .slice(slice)
        self.direction = direction
        self.timestamp = timestamp
    }
    
    init(rotation: CubeRotation, direction: CubeMoveDirection, timestamp: Date = Date()) {
        self.moveType = .rotation(rotation)
        self.direction = direction
        self.timestamp = timestamp
    }
    
    init(wide: CubeWideFace, direction: CubeMoveDirection, timestamp: Date = Date()) {
